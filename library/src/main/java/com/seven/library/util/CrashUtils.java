...
public class CrashUtils implements Thread.UncaughtExceptionHandler {
    ...
    public boolean init() {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            File baseCache = BasicApplication.getInstance().getExternalCacheDir();
            if (baseCache == null) return false;
            crashDir = baseCache.getPath() + File.separator + "crash" + File.separator;
        } else {
            File baseCache = BasicApplication.getInstance().getCacheDir();
            if (baseCache == null) return false;
            crashDir = baseCache.getPath() + File.separator + "crash" + File.separator;
        }
        try {
            PackageInfo pi = BasicApplication.getInstance().getPackageManager().getPackageInfo(BasicApplication.getInstance().getPackageName(), 0);
